...
class Edgarparser(object):
  ...
  def __parseevent(self, html):
    ...
    try:
      ...
      while html != None:
        ...
        # address has low priority; if other item "start texts" are found above, let them override
          ...
          whicSection = def14aSection.NONE
    ...
    first_pass = True
    good_res_nums = []
    new_resolution = collections.OrderedDict()
    for res in resolution:
        if first_pass:
            last_res_num = int(res)
            good_res_nums.append(int(res))
            first_pass = False
        else:
            if last_res_num+1 != int(res):
                for good_res in good_res_nums:
                    new_resolution[good_res] = resolution[str(good_res)]
                break
            else:
                good_res_nums.append(res)
                last_res_num = int(res)
        # print(res, resolution[res])
    print(event_date)
    print(address)
    for new_res in new_resolution:
        print(new_res, new_resolution[new_res])
  ...
  def def14a(self, html):
    ...
    resolution = self.__parseevent(meetingAnnouncement)
